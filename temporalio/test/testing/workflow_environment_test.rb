  ...
  class WorkflowEnvironmentTest < Test
    ...
    def test_time_skipping_auto
      ...
      Temporalio::Testing::WorkflowEnvironment.start_time_skipping(logger: Logger.new($stdout)) do |env|
        ...
        worker.run do
          ...
          assert_in_delta Time.now, env.current_time, 30
          ...
          assert_in_delta Time.now + SlowWorkflow::TWO_DAYS, env.current_time, 30
        end
      end
    end
    ...
    def test_time_skipping_manual
      ...
      Temporalio::Testing::WorkflowEnvironment.start_time_skipping(logger: Logger.new($stdout)) do |env|
        ...
        worker.run do
          ...
          assert_in_delta Time.now, Time.at(handle.query(SlowWorkflow.current_timestamp)), 30

          # Sleep for two hours then signal then check query again
          two_hours = 2 * 60 * 60
          env.sleep(two_hours)
          handle.signal(SlowWorkflow.some_signal)
          assert_in_delta Time.now + two_hours, Time.at(handle.query(SlowWorkflow.current_timestamp)), 30
        end
      end
    end
    ...
    class HeartbeatTimeoutActivity < Temporalio::Activity::Definition
      ...
      def execute
        ...
        timeout = Temporalio::Activity::Context.current.info.heartbeat_timeout
        @env.sleep(timeout * 2)
        'all done'
      end
    end
