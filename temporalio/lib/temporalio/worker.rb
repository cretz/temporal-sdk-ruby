...
module Temporalio
  ...
  class Worker
    ...
    def self.default_illegal_workflow_calls
      @default_illegal_workflow_calls ||= begin
        hash = {
          ...
          'Socket' => :all,
          'Tempfile' => :all,
          'Thread' => %i[abort_on_exception= exit fork handle_interrupt ignore_deadlock= kill new pass pending_interrupt?
                         report_on_exception= start stop initialize join name= priority= raise run terminate
                         thread_variable_set wakeup],
    ...
    def initialize(
      ...
      debug_mode: %w[true 1].include?(ENV['TEMPORAL_DEBUG']&.downcase)
    )
